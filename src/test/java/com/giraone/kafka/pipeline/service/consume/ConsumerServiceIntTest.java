...
abstract class ConsumerServiceIntTest extends AbstractKafkaIntTest {
    ...
    void receiveMultipleEvents() throws Exception {
        ...
        if (AcknowledgementQueueMode.COMPACT.name().equals(applicationProperties.getConsumer().getAcknowledgementQueueMode())) {
            assertThat(afterCommitted - beforeCommitted).isGreaterThan(0);
        } else {
